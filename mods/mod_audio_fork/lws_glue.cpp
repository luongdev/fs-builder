...
namespace {
  ...
  static const char* mySubProtocolName = std::getenv("MOD_AUDIO_FORK_SUBPROTOCOL_NAME") ?
    std::getenv("MOD_AUDIO_FORK_SUBPROTOCOL_NAME") : "default";
    ...
    if (json) {
      ...
      if (0 == type.compare("playAudio")) {
        if (jsonData) {
          ...
          if (!strcmp(szAudioContentType, "raw")) {
          ...
          else if (!strcmp(szAudioContentType, "wave") || !strcmp(szAudioContentType, "wav")) {
            strcpy(fileType, ".wav");
          }
          else {
            validAudio = 0;
          ...
          if (validAudio) {
            ...
            switch_ivr_play_file(session, NULL, szFilePath, NULL);
          }
